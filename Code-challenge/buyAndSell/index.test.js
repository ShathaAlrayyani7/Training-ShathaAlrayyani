...
const maximumProfit = function(array) {
  let minNum = getMinOfArray(array);
  let minIndex = array.indexOf(minNum);
  newArr = array.slice(minIndex);
  let maxNum = getMaxOfArray(newArr);
  if(maxNum > minNum){
    return `buy at ${minNum} and sell at ${maxNum}`;
  }else{
    return "maximum profit that could be earned is 0.";
  }
};


let arr1 = [100, 80, 70, 65, 60, 55, 50];
let arr2 = [100, 80, 120, 130, 70, 60, 100, 125];
console.log(maximumProfit(arr2));
